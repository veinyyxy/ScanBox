...
ScanForChrome::ScanForChrome(QWidget *parent)
...
void ScanForChrome::slot_colorSelected(const QColor &color)
{
	emit ColorValue(color.rgba());
}

int ScanForChrome::newScan(const QString& nothing)
{
...
int ScanForChrome::uploadFile(const QString & strFile)
{
...
int WriteDate(void *buffer, size_t sz, size_t nmemb, void *ResInfo)
{
	...
	return (int)(sz * nmemb);  //返回接受数据的多少
}

QString ScanForChrome::uploadAnyFile(const QString & strURL, const QString & strLocalFile)
{
...
int ScanForChrome::getFileIndex(const QString & strFile)
{
	QVariant var = -1;
...
int ScanForChrome::adjustFile(int front, int back)
{
	int param[2] = { 0 };
	param[0] = front;
	param[1] = back;
...
int ScanForChrome::getCurrentIndex()
{
...
int ScanForChrome::getFileCount()
{
...
int ScanForChrome::setScanFolder(QString strPath)
{
	QVariant result = -1;
...
QString ScanForChrome::getFileInfo(int iIndex)
{
...
QString ScanForChrome::getFileInfoFromNames(QString strFileFullName)
{
...
int ScanForChrome::insertExternalFile(QString strPath, bool isBehind, bool isCopy)
{
...
int ScanForChrome::insertExternalFiles(const QString & strFilesList, bool isBehind, bool isCopy)
{
	char* param = new char[BufferSize];
	memset(param, 0, BufferSize);
	//char param[1024 * 1024 * 2] = { 0 };
...
int ScanForChrome::replaceCurrentFile(const QString& strPath, bool isCopy)
{
	if (strPath.length() > 252)
	{
		return 0;
...
QString ScanForChrome::getDiskFile(const QString &caption, const QString& strDir, const QString& filter)
{
...
QString ScanForChrome::writeTextToImage(int iIndex, int x, int y, uint cololr, uint size, double angle, bool isBold, bool isUnderline, bool StrikeOut, const QString& font, const QString& text)
{
...
QString ScanForChrome::drawImageToImage(int iIndex, int x, int y, bool rotCenter, double rot, double scale, const QString & strWatermark)
{
	QString strSrcImage = getFileFullName(iIndex);
	if (strSrcImage.length() == 0) return 0;
...
uint ScanForChrome::getColor(uint color)
{
	QColor selectColor;
...
QString ScanForChrome::getFont(uint size, bool isBold, bool isUnderline, bool StrikeOut, const QString& font)
{
	bool ok;
...
QString ScanForChrome::rotateImage(int iIndex, double rot)
{
	QString strSrcImage = getFileFullName(iIndex);
	if (strSrcImage.length() == 0) return 0;
...
QString ScanForChrome::sharpenImage(int iIndex)
{
	QString strSrcImage = getFileFullName(iIndex);
	if (strSrcImage.length() == 0) return 0;
...
int ScanForChrome::rectifyImage(int iIndex, double param)
{
	QString strSrcImage = getFileFullName(iIndex);
	if (strSrcImage.length() == 0) return 0;
...
QString ScanForChrome::clipImage(int iIndex, int x, int y, int width, int height)
{
	QString strSrcImage = getFileFullName(iIndex);
	if (strSrcImage.length() == 0) return 0;
...
QString ScanForChrome::mosaicImage(int iIndex, int x, int y, int width, int height, double scale)
{
	QString strSrcImage = getFileFullName(iIndex);
	if (strSrcImage.length() == 0) return 0;
...
#if 0
int ScanForChrome::writeTextToImage(int iIndex, int x, int y, uint cololr, uint size, bool isBold, const QString& font, const QString& text)
{
	char param[1024 * 104] = { 0 };
...
int ScanForChrome::drawImageToImage(int iIndex, int x, int y, const QString& strWatermark)
{
	char param[1024 * 104] = { 0 };
...
#endif
QString ScanForChrome::getFileFullName(int iIndex)
{
...
QString ScanForChrome::GenerateMicrograph(const QString & fileName, float scala)
{
...
bool ScanForChrome::DeleteMicroGraph(const QString & fileName)
{
...
void ScanForChrome::ShowColorDialog(uint color)
{
	m_pColorDialog->setCurrentColor(QColor(color));
	m_pColorDialog->show();
}

uint ScanForChrome::GetSelectColor()
{
...
QString ScanForChrome::getDefaultDevice()
{
	QVariant var = m_pClientThread->invokeFuntion(NAME_getDefaultDevice, 0, 0, m_iConnectTimeout, m_iWriteTimeout, m_iReadTimeout);
	return var.toString();
}

QString ScanForChrome::getRemoveInvokeError()
{
	return m_pClientThread->GetRemoveInvokeError();
}

void ScanForChrome::setConnectTimeout(int iT)
{
	m_iConnectTimeout = iT;
}

void ScanForChrome::setWriteTimeout(int iT)
{
	m_iWriteTimeout = iT;
}

void ScanForChrome::setReadTimeout(int iT)
{
	m_iReadTimeout = iT;
}

int ScanForChrome::saveImageAs(const QString & strSrcName, const QImage& srcImage, QString* strAsName)
{
...
// 通过霍夫变换计算角度vc
double ScanForChrome::CalcDegree(Mat& srcImage, int proValue)
{
...
//逆时针旋转图像degree角度（原尺寸）    
void ScanForChrome::CV_rotateImage(Mat src, Mat& img_rotate, double degree)
{
	//旋转中心为图像中心    
	Point2f center;
