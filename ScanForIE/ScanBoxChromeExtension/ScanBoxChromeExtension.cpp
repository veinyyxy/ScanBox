...
ScanBoxChromeExtension::ScanBoxChromeExtension() : m_pClientThread(new ClientThread)
{
}


ScanBoxChromeExtension::~ScanBoxChromeExtension()
{
}

int ScanBoxChromeExtension::newScan(const QString& nothing)
{
...
int ScanBoxChromeExtension::deleteImage2(const QString& strIndexes)
{
	QVariant var = -1;
	int iLen = strIndexes.length();
	if (0 < iLen && iLen < BufferSize - 100)
	{
		std::string strText = strIndexes.toStdString();
		const char* pBuffer = strText.c_str();
		var = m_pClientThread->invokeFuntion(NAME_deleteImage2, (char*)pBuffer, strIndexes.size() + 1, m_iConnectTimeout, m_iWriteTimeout, m_iReadTimeout);
	}

	return var.toInt();
}

int ScanBoxChromeExtension::uploadFile(const QString & strFile)
{
...
size_t WriteDate(void *buffer, size_t sz, size_t nmemb, void *ResInfo)
{
...
QString ScanBoxChromeExtension::uploadAnyFile(const QString & strURL, const QString & strLocalFile)
{
...
int ScanBoxChromeExtension::getFileIndex(const QString & strFile)
{
	QVariant var = -1;
...
int ScanBoxChromeExtension::adjustFile(int front, int back)
{
	int param[2] = { 0 };
	param[0] = front;
	param[1] = back;
...
int ScanBoxChromeExtension::getCurrentIndex()
{
...
int ScanBoxChromeExtension::getFileCount()
{
...
int ScanBoxChromeExtension::setScanFolder(QString strPath)
{
	QVariant result = -1;
...
QString ScanBoxChromeExtension::getFileInfo(int iIndex)
{
...
QString ScanBoxChromeExtension::getFileInfoFromNames(QString strFileFullName)
{
...
QString ScanBoxChromeExtension::ScalaImage(const QString & sourceFile, float scala)
{
...
int ScanBoxChromeExtension::insertExternalFile(QString strPath, bool isBehind, bool isCopy)
{
...
int ScanBoxChromeExtension::insertExternalFiles(const QString & strFilesList, bool isBehind, bool isCopy)
{
	char* param = new char[BufferSize];
	memset(param, 0, BufferSize);
	//char param[1024 * 1024 * 2] = { 0 };
...
int ScanBoxChromeExtension::replaceCurrentFile(const QString& strPath, bool isCopy)
{
	if (strPath.length() > 252)
	{
		return 0;
...
QString ScanBoxChromeExtension::getDiskFile(const QString &caption, const QString& strDir, const QString& filter)
{
...
QString ScanBoxChromeExtension::writeTextToImage(int iIndex, int x, int y, uint cololr, uint size, double angle, bool isBold, bool isUnderline, bool StrikeOut, const QString& font, const QString& text)
{
...
QString ScanBoxChromeExtension::drawImageToImage(int iIndex, int x, int y, bool rotCenter, double rot, double scale, const QString & strWatermark)
{
	QString strSrcImage = getFileFullName(iIndex);
	if (strSrcImage.length() == 0) return 0;
...
uint ScanBoxChromeExtension::getColor(uint color)
{
	QColor selectColor;
...
uint ScanBoxChromeExtension::getColor()
{
...
QString ScanBoxChromeExtension::getFont()
{
	bool ok;
...
QString ScanBoxChromeExtension::getFont(uint size, bool isBold, bool isUnderline, bool StrikeOut, const QString& font)
{
	bool ok;
...
QString ScanBoxChromeExtension::rotateImage(int iIndex, double rot)
{
	QString strSrcImage = getFileFullName(iIndex);
	if (strSrcImage.length() == 0) return 0;
...
QString ScanBoxChromeExtension::sharpenImage(int iIndex)
{
	QString strSrcImage = getFileFullName(iIndex);
	if (strSrcImage.length() == 0) return 0;
...
int ScanBoxChromeExtension::rectifyImage(int iIndex, double param)
{
	QString strSrcImage = getFileFullName(iIndex);
	if (strSrcImage.length() == 0) return 0;
...
QString ScanBoxChromeExtension::clipImage(int iIndex, int x, int y, int width, int height)
{
	QString strSrcImage = getFileFullName(iIndex);
	if (strSrcImage.length() == 0) return 0;
...
QString ScanBoxChromeExtension::mosaicImage(int iIndex, int x, int y, int width, int height, double scale)
{
	QString strSrcImage = getFileFullName(iIndex);
	if (strSrcImage.length() == 0) return 0;
...
#if 0
int ScanBoxChromeExtension::writeTextToImage(int iIndex, int x, int y, uint cololr, uint size, bool isBold, const QString& font, const QString& text)
{
	char param[1024 * 104] = { 0 };
...
int ScanBoxChromeExtension::drawImageToImage(int iIndex, int x, int y, const QString& strWatermark)
{
	char param[1024 * 104] = { 0 };
...
#endif
QString ScanBoxChromeExtension::getFileFullName(int iIndex)
{
...
QString ScanBoxChromeExtension::GenerateMicrograph(const QString & fileName, float scala)
{
...
bool ScanBoxChromeExtension::DeleteMicroGraph(const QString & fileName)
{
...
void ScanBoxChromeExtension::ShowColorDialog(uint color)
{
	m_pColorDialog->setCurrentColor(QColor(color));
	m_pColorDialog->show();
}

uint ScanBoxChromeExtension::GetSelectColor()
{
...
QString ScanBoxChromeExtension::getDefaultDevice()
{
	QVariant var = m_pClientThread->invokeFuntion(NAME_getDefaultDevice, 0, 0, m_iConnectTimeout, m_iWriteTimeout, m_iReadTimeout);
	return var.toString();
}

QString ScanBoxChromeExtension::getRemoveInvokeError()
{
	return m_pClientThread->GetRemoveInvokeError();
}

void ScanBoxChromeExtension::setConnectTimeout(int iT)
{
	m_iConnectTimeout = iT;
}

void ScanBoxChromeExtension::setWriteTimeout(int iT)
{
	m_iWriteTimeout = iT;
}

void ScanBoxChromeExtension::setReadTimeout(int iT)
{
	m_iReadTimeout = iT;
}

int ScanBoxChromeExtension::saveImageAs(const QString & strSrcName, const QImage& srcImage, QString* strAsName)
{
...
// 通过霍夫变换计算角度vc
double ScanBoxChromeExtension::CalcDegree(Mat& srcImage, int proValue)
{
...
//逆时针旋转图像degree角度（原尺寸）    
void ScanBoxChromeExtension::CV_rotateImage(Mat src, Mat& img_rotate, double degree)
{
	//旋转中心为图像中心    
	Point2f center;
