...
void WriteLogging(const QString& name, const QString& text)
{
	QFile logFile(name);
...
ClientThread::ClientThread() : m_strLoggingPath("C:\\windows\\temp\\ScanForIE_logging.txt")
{
...
QVariant ClientThread::invokeFuntion(char * funName, char * param, size_t len, int iConnectTimeout, int iWriteTimeout, int iReadTimeout)
{
	m_strError = "success";
	QString loggingText = QString::fromLocal8Bit("[%1] {To invoke remote function (name : %2, parameters : %3)} -> {function return (%4)}");
	...
	if (m_bWriteLogging)
	{
		...
		loggingText = loggingText.arg(strTime).arg(QString::fromLocal8Bit(funName)).arg("ignore......").arg(result.toString());
		
		WriteLogging(m_strLoggingPath, loggingText);
	}
	////////////////////////////////////////////////////////////////////////
	return result;
}
